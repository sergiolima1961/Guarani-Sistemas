...
type
  ...
  type
    ...
    DBGrid1: TDBGrid;
      ...
      procedure DS_FORNECEDORDataChange(Sender: TObject; Field: TField);
...
procedure TfCadFornecedor.Button1Click(Sender: TObject);
begin
  ...
  DM_testepratico.FDFornecedor.Append;
  DM_testepratico.FDENDERECO.Append;

  DM_testepratico.FDFornecedor.FieldByName('ID_ENTIDADE').Value :=
...
procedure TfCadFornecedor.Button2Click(Sender: TObject);
begin
  DM_testepratico.FDFornecedor.Cancel;
...
procedure TfCadFornecedor.Button3Click(Sender: TObject);
var
  gera_email: Boolean;
begin
  try
    DM_testepratico.fdcDB.StartTransaction;
    DM_testepratico.FDFornecedor.Post;
  ...
  // Preparo_Email(DM_testepratico.FDFornecedorID_ENTIDADE.Value);
end;

procedure TfCadFornecedor.Button4Click(Sender: TObject);
begin
  Button1.Enabled := false;
  Button4.Enabled := (Button1.Enabled);
  Button5.Enabled := (Button1.Enabled);
  ...
  Button2.Enabled := not(Button1.Enabled);
  DBNavigator1.Enabled := not(Button1.Enabled);
  DM_testepratico.FDFornecedor.Edit;
  DM_testepratico.FDENDERECO.Edit;
end;

procedure TfCadFornecedor.Button5Click(Sender: TObject);
begin
  try
      DM_testepratico.FDFornecedor.Edit;
      DBGrid1.Tag := DM_testepratico.FDFornecedor.FieldByName('ID_ENTIDADE').Value  ;

      if DM_testepratico.FDFornecedor.FieldByName('STATUS').Value=0 then
      begin
        if MessageDlg('Confirma a Inativação ?', TMsgDlgType.mtConfirmation,
          [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo], 0) = idYes then
         DM_testepratico.FDFornecedor.FieldByName('STATUS').Value := 1;
      end else
      if DM_testepratico.FDFornecedor.FieldByName('STATUS').Value=1 then
      begin
        if MessageDlg('Confirma a Ativação ?', TMsgDlgType.mtConfirmation,
          [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo], 0) = idYes then
         DM_testepratico.FDFornecedor.FieldByName('STATUS').Value := 0;
      end;
      ...
      DM_testepratico.FDFornecedor.Locate('ID_ENTIDADE',DBGrid1.Tag,[]);
  end;

end;

procedure TfCadFornecedor.Button_Buscar_CepClick(Sender: TObject);
begin
  try    if Length(DBEdit11.Text) <> 8 then
    begin
...
procedure TfCadFornecedor.FDQuery1AfterCancel(DataSet: TDataSet);
begin
  Button1.Enabled := true;
  Button3.Enabled := Not(Button1.Enabled);
  Button2.Enabled := Not(Button1.Enabled);
...
procedure TfCadFornecedor.FDQuery1AfterEdit(DataSet: TDataSet);
begin
  Button3.Enabled := Not(Button1.Enabled);
  Button2.Enabled := Not(Button1.Enabled);
  DBNavigator1.Enabled := (Button1.Enabled);
...
procedure TfCadFornecedor.FDQuery1AfterInsert(DataSet: TDataSet);
begin
...
procedure TfCadFornecedor.FDQuery1AfterPost(DataSet: TDataSet);
begin
  Button1.Enabled := true;
  Button3.Enabled := Not(Button1.Enabled);
  Button2.Enabled := Not(Button1.Enabled);
...
procedure TfCadFornecedor.FDQuery1AfterScroll(DataSet: TDataSet);
begin
...
procedure TfCadFornecedor.FDQuery1EMAILValidate(Sender: TField);
var
  rRegex: TRegex;
  bRet: Boolean;
begin
...
procedure TfCadFornecedor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  DM_testepratico.FDFornecedor.Close;
...
begin
  ...
  if not DM_testepratico.FDFornecedor.FieldByName('ID_ENTIDADE').IsNull then
    DM_testepratico.FDENDERECO.Params.ParamByName('ID_ENTIDADE').Value :=
      DM_testepratico.FDFornecedor.FieldByName('ID_ENTIDADE').Value;
  DM_testepratico.FDENDERECO.OPEN;
end;

function TfCadFornecedor.GeneratorID(aName: string;
...
procedure TfCadFornecedor.ConsultaCEP(CEP: String);
var
  tempXML: IXMLNode;
...
procedure TfCadFornecedor.DS_FORNECEDORDataChange(Sender: TObject;
  Field: TField);
begin
  Label16.Caption := FormatFloat('0###', DM_testepratico.FDFornecedor.RecNo) +
    '/' + FormatFloat('0###', DM_testepratico.FDFornecedor.RecordCount);

end;

procedure TfCadFornecedor.DBEdit11KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfCadFornecedor.DBRadioGroup1Exit(Sender: TObject);
begin
  if DM_testepratico.FDFornecedorPESSOA.Value = 'F' then
  Begin
    DM_testepratico.FDFornecedorINSCRICAO_FISCAL.EditMask :=
...
procedure TfCadFornecedor.mensagemAviso(mensagem: string);
begin
  Application.MessageBox(PChar(mensagem), '', MB_OK + MB_ICONERROR);
end;

procedure TfCadFornecedor.Preparo_Email(id: integer);
var
...
begin
  ...
  NodeTabela := XMLDocument.AddChild('Fornecedores');
  NodeRegistro := NodeTabela.AddChild('Registro');
  NodeRegistro.ChildValues['Nome'] := DM_testepratico.FDFornecedorNOME.Value;
  NodeRegistro.ChildValues['Nascimento'] :=
    DM_testepratico.FDFornecedorNASCIMENTO.AsString;
  NodeRegistro.ChildValues['InscricaoFiscal'] :=
    DM_testepratico.FDFornecedorINSCRICAO_FISCAL.Value;
  NodeRegistro.ChildValues['Pessoa'] :=
    DM_testepratico.FDFornecedorPESSOA.Value;
  NodeRegistro.ChildValues['Telefone'] :=
    DM_testepratico.FDFornecedorTELEFONE.Value;
  NodeRegistro.ChildValues['Email'] :=
    DM_testepratico.FDFornecedorTELEFONE.Value;
  ...
  aanexo := ExtractFilePath(Application.ExeName) +
    DM_testepratico.FDFornecedorINSCRICAO_FISCAL.Value + '.xml';

  XMLDocument.SaveToFile(aanexo);

  EnviarEmail('Cadastro de Fornecedor', aanexo);
end;

procedure TfCadFornecedor.EnviarEmail(AAssunto, aanexo: String);
var
  idMsg: TIdMessage;
  IdText: TIdText;
  sAnexo: string;
begin
  try
    ...
    try
      ...
      idMsg.From.Name := 'Cadastro fornecedor';
      idMsg.From.Address := DM_testepratico.FDFornecedorEMAIL.AsString;
      // sFrom;
      ...
      idMsg.Recipients.EMailAddresses :=
        DM_testepratico.FDFornecedorEMAIL.AsString;
      ...
      // backup do email
      idMsg.SaveToFile(DM_testepratico.FDFornecedorINSCRICAO_FISCAL.Value
        + '.eml');
